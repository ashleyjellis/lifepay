...
import { fetchAccounts, refreshUserToken } from '@/lib/tink';
...
export async function GET() {
  ...
  // Check if a Tink user row exists at all (even without tokens yet)
  const db = getDb();
  const existing = await db.execute({
    sql: `SELECT access_token FROM bank_connections WHERE user_id=? AND provider='tink'`,
    args: [user.id],
  });
  if (!existing.rows.length || !existing.rows[0].access_token) {
  ...
  try {
    ...
    console.error('Tink accounts error:', e);
