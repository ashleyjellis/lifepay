...
export async function PUT(req: Request) {
  ...
  const bill = await db.execute({ sql: 'SELECT household_id FROM bills WHERE id=?', args: [body.id] });
  if (!bill.rows[0] || !(await assertHousehold(bill.rows[0].household_id as string, user.id))) {
...
export async function DELETE(req: Request) {
  ...
  const db = getDb();
  const bill = await db.execute({ sql: 'SELECT household_id FROM bills WHERE id=?', args: [id] });
  if (!bill.rows[0] || !(await assertHousehold(bill.rows[0].household_id as string, user.id))) {
    return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
  }
